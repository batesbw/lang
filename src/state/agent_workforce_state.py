from typing import TypedDict, Optional, List, Any
from src.schemas.auth_schemas import SalesforceAuthRequest, SalesforceAuthResponse
from src.schemas.flow_schemas import FlowBuildRequest, FlowBuildResponse
from src.schemas.deployment_schemas import DeploymentRequest, DeploymentResponse

class AgentWorkforceState(TypedDict, total=False):
    # Authentication related state
    current_auth_request: Optional[SalesforceAuthRequest]
    current_auth_response: Optional[SalesforceAuthResponse]
    is_authenticated: bool
    salesforce_session: Optional[SalesforceAuthResponse] # Stores active session details

    # Flow Building related state
    current_flow_build_request: Optional[FlowBuildRequest]
    current_flow_build_response: Optional[FlowBuildResponse]

    # Deployment related state (NEW)
    current_deployment_request: Optional[DeploymentRequest]
    current_deployment_response: Optional[DeploymentResponse]
    
    # General state
    messages: Optional[List[Any]] # For storing LangGraph message history
    error_message: Optional[str] # General error messages not specific to an agent's response
    retry_count: int # For managing retries in loops 